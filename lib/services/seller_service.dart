...
class SellerService {
  ...
  Future<SellerModel> createSellerRegistration({
    ...
    try {
      ...
      // Convert vehicles to string array format for Appwrite
      final vehicleStrings = vehicles.map((v) {
        return '${v.vehicleNumber}|${v.documentId ?? ''}';
      }).toList();
      ...
      if (aadharDocumentId != null && aadharDocumentId.isNotEmpty) {
        await deleteDocument(aadharDocumentId);
      }
  ...
  SellerModel _documentToSellerModel(models.Document doc) {
    // Parse vehicles from string array format
    final vehiclesList =
        (doc.data['vehicles'] as List?)?.map((v) {
    ...
    return SellerModel(
      ...
      aadharCardNo: doc.data['aadhar_card_no'] ?? '',
      aadharDocumentId: doc.data['aadhar_document_id']?.isEmpty ?? true
          ? null
          : doc.data['aadhar_document_id'],
